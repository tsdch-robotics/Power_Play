...
/*
    ...
    public void forward(DcMotor motor1, DcMotor motor2, DcMotor motor3, DcMotor motor4, DcMotor OD, int targ){
        ...
        while (OD.getCurrentPosition() < targ && (Math.abs(targ - OD.getCurrentPosition())) > 2000) {// targ - current
            motor1.setPower(-allTimeSpeed);
            motor2.setPower(-allTimeSpeed);
            motor3.setPower(-allTimeSpeed + .11);
            motor4.setPower(-allTimeSpeed + .11);
        }

        while (OD.getCurrentPosition() < targ && (Math.abs(targ - OD.getCurrentPosition())) < 10000) {// targ - current
        ...
        while (OD.getCurrentPosition() < targ && (Math.abs(targ - OD.getCurrentPosition())) < 5000) {// targ - current
            motor1.setPower(-allTimeSpeed + .2);//make sure not zero, no less than .25 typical speed
            motor2.setPower(-allTimeSpeed + .2);
            motor3.setPower(-allTimeSpeed + .2);
            motor4.setPower(-allTimeSpeed + .2);
        }

        while (OD.getCurrentPosition() >= targ) {//pos
            motor1.setPower(0);
            motor2.setPower(0);
            motor3.setPower(0);
            motor4.setPower(0);
